...
using Circle.Web.Helper.Bootstrap.Grid.Column;
using Circle.Web.Helper.Extensions;
using Circle.Web.Helper.Types;
...
namespace Circle.Web.Helper.Bootstrap.Grid
{
    public class Grid<T> : IHtmlContent where T : class
    {
        ...
        public Grid<T> SearchInputPlaceHolder(string placeHolder)
        {
        ...
        public void ToHeaderHtmlString(ref StringBuilder sb)
        {
            sb.Append("<div class='d-flex flex-stack mb-5'>");

            sb.Append("<div class='d-flex align-items-center position-relative my-1'>");

            sb.Append("<span class='svg-icon svg-icon-2'>...</span>");

            sb.Append($"<input type='text' data-kt-docs-table-filter='search' class='form-control form-control-solid w-250px ps-15' placeholder='{placeHolder}'/>");

            if (_toolbarButtons.Count > 0)
            {
                sb.Append("<div class='d-flex justify-content-end' data-kt-docs-table-toolbar='base'>");
